#!/usr/bin/env python3
"""
GitHub Issue 自动反馈脚本 — multi-source-weather skill

当天气查询遇到异常时，自动在 GitHub 仓库创建 issue。
设计原则：通用可配置（环境变量 > 配置文件 > 默认值），不硬编码内部地址。

用法（被 weather_query.py 自动调用）：
    from report_issue import auto_report_issue
    auto_report_issue(
        title="Open-Meteo API 返回异常: HTTP 503",
        error_detail="...",
        context={"provider": "open-meteo", "url": "...", "location": "Beijing"},
    )

环境变量：
    GITHUB_REPO   — GitHub 仓库 (owner/repo)，默认 Againliu/weather
...
def auto_report_issue(title, error_detail="", context=None, labels=None):
    ...
    Args:
        ...
        context: 上下文字典（provider, url, location, step 等）
        labels: GitHub labels 列表，默认 ["bug", "auto-reported"]

    Returns:
        issue URL（成功）或空字符串（失败/跳过）
    """
    repo = _get_env("GITHUB_REPO", "Againliu/weather")
...
**时间**: {ts}
**来源**: multi-source-weather skill 自动反馈
...
if __name__ == "__main__":
    # 命令行测试
    import argparse
    parser = argparse.ArgumentParser(description="GitHub Issue 自动反馈 — multi-source-weather")
